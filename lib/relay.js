...
var _opt = _optimist.usage('Graphdat Relay')
	...
	.options('p', {
		describe : 'Directory for plugins, default is ' + _path.join(__dirname, 'plugins'),
		alias : 'plugins'
	})
	.wrap(80);
...
var _ejectedCount = 0;
var _sendFailed;
var _confPath = _argv.config ? _path.join(process.cwd(), _argv.config) : _path.join(__dirname, 'config.json');
var _invalidPluginPath = _path.join(__dirname, 'plugins-archive');
var _pluginPath = _argv.plugins || _path.join(__dirname, 'plugins');
...
function spawn(pi, command, cbClose, cbOutput)
{
	...
	var prc = _childProcess.spawn(parts[0], parts.slice(1), {cwd : pi.dir, env : process.env});
...
function ensureProcess(pi)
{
	...
	pi.prc = spawn(pi, pi.command, function(code)
	{
		pi.prc = null;

		if (code && !pi.ignoreErrors && !pi.disabled)
		{
			pi.runErrorCount++;
...
{
	return _conf.disabled || _plugins[pname.toLowerCase()].disabled || getPluginParam(pname, 'disabled');
}
...
function refreshPlugins(cb)
{
	...
	if (plugins)
	{
		function doit()
		{
			...
			if (!fExists || fDisabled || fForceDisable)
			{
				_log.info('Plugin %s has been %s, stopping', name, fForceDisable ? 'forced closed by relay' : (!fExists ? 'removed' : 'disabled'));
