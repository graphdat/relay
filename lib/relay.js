...
function refreshPlugins(cb)
{
	...
	function loadPlugins()
	{
		...
		_async.series(funcs, function(err)
		{
			...
			queueLength *= 10; // average number of sources
			...
			newQueue.push.apply(null, oldQueue.toArray());
			newQueue.overflow = handleEject;

			// remap the _outboundMap
			var oldMap = _outboundMap;
			var newMap = {};
			newQueue.forEach(function(item) { newMap[item.source + ':' + item.metric + ':' + item.ts] = item; });
	...
	if (plugins)
	{
		function doit()
		{
			...
			if ((!fExists || fDisabled || fForceDisable) && _plugins[name].prc)
			{
				...
				_plugins[name].ignoreErrors = true;
