...
var _fWINDOWS = (process.platform === "win32");
...
function _spawnChildProcess(cwd, exe, params)
{
	var config = {cwd : cwd, env : process.env};
	if (_fWINDOWS)
	{
		var command = "cmd";
		var commandParams = ["/C", exe].concat(params);
		return _childProcess.spawn(command, commandParams, config);
	}
	else
	{
		return _childProcess.spawn(exe, params, config);
	}
}
...
function spawn(pi, command, cbClose, cbOutput)
{
	...
	var prc = _spawnChildProcess(pi.dir, exe, params);
