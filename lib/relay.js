...
var _fSUDO = (process.env['USER'] === 'root' && process.env['SUDO_USER'] !== undefined && process.stdout.isTTY);
...
var _defaultConfig =
{
	...
	gcInterval : 5000,
	sendCompress : true,
	sendHost : 'premium-api.boundary.com',
	sendBacklog : 200,
	sendClear : false,
...
var RESERVED_PLUGIN_PARAMS = { pollInterval: { type: 'integer', default: 1000, minimum: 1000 } };
...
/***
 * Check for a parsable timestamp
 *
 * @param timestamp
 * @returns {boolean}
 */
function isTimestamp(timestamp)
{
	...
	return (new Date(parseInt(timestamp, 10))).getYear() >= CURRENT_YEAR;
}
...
function spawn(pi, command, cbClose, cbOutput)
{
	...
	// handle linux AND windows commands
	var cmd = _which.sync(exe);
	var prc = _childProcess.spawn(cmd, params, {cwd : pi.dir, env : process.env});
 ...
 *
 * @param pname
 * @returns false or text describing why it is disabled (worded to follow the word 'because')
 */
function pluginDisabled(pname)
{
	pname = pname.toLowerCase();

	var pi = _plugins[pname];

	if (_conf.disabled)
		return 'the relay is disabled';
	else if (pi && pi.disabled)
		return 'it is disabled';
	else if (getPluginParam(pname, 'disabled'))
		return 'it is remotely disabled';
	else if (pi && pi.runErrorCount && pi.runErrorCount >= RUN_ERROR_THRESH)
		return 'the run fail rate has exceeded the threshold';

	var conf = _conf && _conf.config && _conf.config.plugins && _conf.config.plugins[pname];

	if (!conf)
		return 'there is no configuration';

	if (conf._metadata.paramArray && (!conf.items || !conf.items.length))
		return 'no configurations are present';
...
function getReservedParams(params, pi)
{
	var reserved = {};

	_.each(RESERVED_PLUGIN_PARAMS, function(options, name)
	{
		var value = (params && params[name]) || (pi && pi[name]);
		...
		else if (options.type === 'integer')
		{
			if (!isNumber(value))
				value = options.default;
			else if (options.minimum && value < options.minimum)
				value = options.minimum;
			else if (options.maximum && value > options.maximum)
...
{
	var resp = getReservedParams(param, pi);
...
function verifyPlugin(name, cbVerify)
{
	...
	], function(err)
	{
		if (err)
		{
			pluginErr(pi, 'Unexpected error: %s', err);
			pluginErr(pi, 'The plugin is disabled.  Re-enable plugin to try again.');
...
function getPluginParams(pname)
{
	...
	// Add in reserved
	var resp = getReservedParams(params, pi);
...
function loadPlugin(pname, cb)
{
	...
	function finish()
	{
		...
		catch(errmsg)
		{
			...
			pluginErr(plugin, 'Disabling the plugin.  Re-enable the plugin to try again.');
			disablePlugin(pname, true);
		}
...
function refreshPlugins(cb)
{
	...
	function loadPlugins()
	{
		...
		{
			// if the relay is disabled, reset the plugin error count
			_.each(_plugins, function(pi, name)
			{
				if (pi && pi.runErrorCount)
					pi.runErrorCount = 0;
			});
		...
		for(var name in _conf.config.plugins)
		{
			...
			{
				_log.info('Skipping plugin %s, because %s', name, res);
			}
		}
	...
	if (plugins)
	{
		function doit()
		{
			...
			if (res && _plugins[name].prc)
			{
				_log.info('Stopping plugin %s because %s', name, res);

				killProcess(_plugins[name], function()
				{
					if (!_conf || !_conf.config || !_conf.config.plugins || !_conf.config.plugins[name.toLowerCase()])
...
function handleConfig(cfg, cb)
{
	...
	if (cfg)
	{
		...
		// Special case: if we are to be enabling/disabling plugins, transfer the flag
		// to overwrite the in memory defaults
		for(var pname in cfg.config.plugins)
		{
			var picfg = cfg.config.plugins[pname];
			var pi = _plugins[pname.toLowerCase()];

			if (pi && picfg && picfg.disabled !== pi.disabled)
					pi.disabled = picfg.disabled;
		}
...
_single.check(function(err)
{
	...
	{
		var rl = _readline.createInterface({ input: process.stdin, output: process.stdout });
